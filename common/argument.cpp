...
namespace hworld {
	...
	bool Argument::Parse(int argc, char *argv[]) {
		if (argc > 1) {
            ...
            if (s == "--console-with-cmd") {
				...
				if (argc > 2) {
					printf("%s\n", (utils::Aes::CryptoHex(argv[2], hworld::GetDataSecuretKey())).c_str());
				}
				else {
			...
			//	printf("%s\n", utils::Aes::HexDecrypto(argv[2], hworld::GetDataSecuretKey()).c_str());
			//	return true;
			//}
			else if (s == "--sm3" ) {
				if (argc > 2) {
				...
				std::string sk_hash = utils::String::BinToHexString(hworld::HashWrapper::Crypto(hworld::GetDataSecuretKey())).substr(0, 2);
#ifdef SVNVERSION
				printf("%s;%u;secure:%s;git:" SVNVERSION "\n", General::hworld_VERSION, General::LEDGER_VERSION, sk_hash.c_str());
#else
				printf("%s;%u;secure:%s\n", General::hworld_VERSION, General::LEDGER_VERSION, sk_hash.c_str());
#endif 
				return true;
				...
				result["private_key_aes"] = utils::Aes::CryptoHex(private_key, hworld::GetDataSecuretKey());
				...
				hworld::PublicKey pub_key(argv[2]);
	...
	void Argument::ShowNodeId(int argc, char *argv[]) {
		...
		hworld::PrivateKey private_key(utils::Aes::HexDecrypto(argv[2], hworld::GetDataSecuretKey()));
