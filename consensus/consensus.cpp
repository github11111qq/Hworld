...
// 		return UpdateValidators(proto_validators);
	...
	bool Consensus::SaveValue(const std::string &name, const std::string &value) {
		...
		return db->Put(utils::String::Format("%s_%s", hworld::General::CONSENSUS_PREFIX, name.c_str()), value);
	}
	...
	int32_t Consensus::LoadValue(const std::string &name, std::string &value) {
		KeyValueDb *db = Storage::Instance().keyvalue_db();
		return db->Get(utils::String::Format("%s_%s", hworld::General::CONSENSUS_PREFIX, name.c_str()), value) ? 1 : 0;
	}

	bool Consensus::DelValue(const std::string &name) {
		KeyValueDb *db = Storage::Instance().keyvalue_db();
		return db->Delete(utils::String::Format("%s_%s", hworld::General::CONSENSUS_PREFIX, name.c_str())) ? 1 : 0;
	}
	...
	void ValueSaver::SaveValue(const std::string &name, const std::string &value) {
		writes.Put(utils::String::Format("%s_%s", hworld::General::CONSENSUS_PREFIX, name.c_str()), value);
		write_size++;
